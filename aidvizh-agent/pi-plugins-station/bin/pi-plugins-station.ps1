...
<#
.SYNOPSIS
    pi-plugins-station on Windows: install third-party pi plugins from the station catalog.

.DESCRIPTION
    Thin wrapper around bin/pi-plugins-station.mjs - one engine for every OS. Paths are derived from the
...
.EXAMPLE
    pwsh -File bin/pi-plugins-station.ps1 list
    pwsh -File bin/pi-plugins-station.ps1 install all
    pwsh -File bin/pi-plugins-station.ps1 status -WhatIf
#>
...
try {
    ...
    $engine = Join-Path $root 'bin/pi-plugins-station.mjs'
