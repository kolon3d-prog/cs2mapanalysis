...
<#
.SYNOPSIS
    spec-station on Windows: install the spec mode (skill spec-mode + slash commands) into clients.

.DESCRIPTION
    Thin wrapper around bin/spec-station.mjs - one engine for every OS. Paths are derived from the
...
.EXAMPLE
    pwsh -File bin/spec-station.ps1 list
    pwsh -File bin/spec-station.ps1 install
    pwsh -File bin/spec-station.ps1 install --layer project --client none -WhatIf
#>
...
try {
    ...
    $engine = Join-Path $root 'bin/spec-station.mjs'
